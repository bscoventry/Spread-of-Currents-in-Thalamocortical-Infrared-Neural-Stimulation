...
if __name__ == '__main__':                                            #This statement is to allow for parallel sampling in windows. Linux distributions don't require this.
    ...
    """
    data = pd.read_pickle("velocitySour.pkl")           #Use pandas to read in data

    """
    Convert power to energy based on laser power levels and pulse widths, then save this into the dataframe
    """
    data1 = data.loc[data['velSour'] >0.001]                    #Responses driven by INS
    ...
    MaxZ = data["velSour"].astype(aesara.config.floatX)               #Convert to tensor
    ...
    plt.show()
    pdb.set_trace()
    az.to_netcdf(rTrace,filename='velSour.netcdf')
    az.to_netcdf(ppc,filename='velSour.netcdf')
